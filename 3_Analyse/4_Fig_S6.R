...
texture_costs <- seq(29, 55, by = 0.025)
...
Sand_economic_cost_plot <- ggplot(
  ...
  scale_y_continuous(
    breaks = seq(30, max(Sand_economic_cost$texture_cost, na.rm = TRUE), by = 7.5),
    expand = c(0,0)
  )+
...
visnir_costs  <- seq(0.75, 10.7, by = 0.025)
Total_C_costs <- seq(15, 25, by = 0.025)
...
visnir_costs  <- seq(0.75, 10.7, by = 0.025)
Al_Meh3_costs <-  seq(12, 22, by = 0.025)
...
K_economic_cost_plot <-ggplot(
  ...
  theme(
    panel.grid = element_blank(),
    text = element_text(family = "Arial"),
    axis.title.x = element_text(size = 20,hjust = 1.9, face = "italic"),
...
visnir_costs  <- seq(0.75, 10.7, by = 0.025)
pH_SMP_costs <- seq(12, 22, by = 0.025)
...
ggsave("Figures/FigS6.jpg", plot = Rejector_Matrix_Economic_cost_Plot_Map_Appendix, width = 3000, height = 2600, units = "px", dpi = 200)
