...
for target in soil_properties_target:
    ...
    tuning_overview_df = pd.read_csv(ROOT / "Results" / "PLSQR" / "Inner_CV" / f"Tuning_Overview_{target}_IS.csv")
    ...
    for i in range(1, k + 1):
        ...
        pls = PLSRegression(n_components= best_row["n_components"])
        pls.fit(X_train, y_train)

        X_train_pls = pls.transform(X_train, y=None, copy=True)
        X_test_pls = pls.transform(X_test, y=None, copy=True)

        # Mean prediction via OLS
        ols = LinearRegression()
        ols.fit(X_train_pls, y_train)
        y_pred_mean = ols.predict(X_test_pls)
        ...
        fold_quantiles = np.column_stack([
            QuantileRegressor(quantile=q_scaled, alpha=0)
            .fit(X_train_pls, y_train)
            .predict(X_test_pls)
        ...
        # model = TabICLRegressor(n_estimators=4)
        # model.fit(X_train, y_train)
        
        
        # quantiles_list_scaled = logit_rescale_alphas(quantiles_list, best_row["Factor"])

        # y_pred_dict = model.predict(
        #     X_test,
        #     output_type=["mean", "quantiles"],
        #     alphas=quantiles_list_scaled
        # )
        ...
        # fold_quantiles = np.asarray(y_pred_dict['quantiles'])
        # y_quantiles_all_folds.append(fold_quantiles)
        
    
    out_dir = ROOT / "Results" / "PLSQR" / "Outer"
