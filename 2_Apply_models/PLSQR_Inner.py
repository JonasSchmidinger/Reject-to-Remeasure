...
def run_iteration(
    ...
    cache_dir = Path("Results/PLSQR/Inner_CV/.cache")
    if use_cache:
    ...
    if cache_file.exists():
        ...
        for l in range(1, L + 1):
            ...
            pls = PLSRegression(n_components=int(Params["n_components"]))
            pls.fit(X_train_tune, y_train_tune)

            X_train_pls = pls.transform(X_train_tune)
            X_calibration_pls = pls.transform(X_calibration_tune)

            ols = LinearRegression()
            ols.fit(X_train_pls, y_train_tune)
            y_pred_mean = ols.predict(X_calibration_pls)

            quantiles_list_scaled = s_factor(quantiles_list, Params["Factor"])
            ...
            y_pred_quantiles = np.column_stack(
                [
                    ...
                    .fit(X_train_pls, y_train_tune)
                    .predict(X_calibration_pls)
                    for q_scaled in quantiles_list_scaled
                ]
            )

            y_pred_quantiles = np.sort(
                y_pred_quantiles, axis=1
            )  # Not sure if we need and whether quantiles cross, in which case it would anyways be kinda a "bad model"

            y_pred_mean_list.append(y_pred_mean.ravel())
            y_pred_quantiles_list.append(y_pred_quantiles)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run PLSQR optimization for a specific soil target."
    )
    parser.add_argument(
        "--target",
        type=str,
    ...
    parser.add_argument(
        "--enable-cache",
        action="store_true",
        help="Enable caching to save/load intermediate predictions.",
    )
    ...
    PLSQR_Grid = pd.read_csv(r"Data/Preprocessing_Grid/preprocessing_grid_PLSQR.csv")
    ...
    for i in range(1, k + 1):
        for hyper_row in range(len(PLSQR_Grid)):
            Params = PLSQR_Grid.iloc[hyper_row]
    ...
    results = Parallel(n_jobs=128, verbose=5)(tasks)

    tuning_overview_df = pd.concat(results, ignore_index=True)

    out_folder = Path("Results/PLSQR/Inner_CV/")
